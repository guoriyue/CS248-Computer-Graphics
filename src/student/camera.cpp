...
Ray Camera::generate_ray(Vec2 screen_coord) const {
    ...
    // To support multi-spectral rendering, now each ray has its own wavelength 
    // lambda. When the camera generates a ray, the ray's wavelength should 
    // be a random float number between 400nm to 690nm. 
