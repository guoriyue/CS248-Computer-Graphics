...
//       world.enclose(primitives[i]->get_bbox());
//     } 
...
// //随机在空间内选点
// vector< Vec3 > Kmeans::getRandCentroids(Vec3  pMin, Vec3  pMax, int k,int p){
//     //初始随机k个representive
//    vector<Vec3 > kCentroid;
...
//    //第一个随机点
//    kCentroid.push_back(Vec3 (randX,randY,randZ));
...
//         tempP.push_back(Vec3 (randX,randY,randZ));
//       }
...
// 	    double distance=length(tempP[k]-kCentroid[q]);
...
// 	    double distance=length(bb.extent);
// 	    if(distance>maxDistance){
// 	       maxDistance=distance;
// 	       index=k;
// 	    }
...
// 	bb.enclose(primitives[cluster[i].indexOfPrimitives[j]]->get_bbox());
...
//       bb.enclose(primitives[a->c.indexOfPrimitives[i]]->get_bbox());
//     }
    
//     for(size_t i=0;i<b->c.indexOfPrimitives.size();++i){
//       c.indexOfPrimitives.push_back(b->c.indexOfPrimitives[i]);
//       bb.enclose(primitives[b->c.indexOfPrimitives[i]]->get_bbox());
//     }
